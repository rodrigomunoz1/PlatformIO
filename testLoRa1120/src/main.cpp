...
void getVersion() {
  ...
  SPI.transfer((CMD_GET_VERSION >> 8) & 0xFF);
  SPI.transfer(CMD_GET_VERSION & 0xFF);
  digitalWrite(NSS_PIN, HIGH);
  waitBusy();
  digitalWrite(NSS_PIN, LOW);
  byte stat1 = SPI.transfer(0x00);
  byte hw_ver = SPI.transfer(0x00);
  byte us = SPI.transfer(0x00);
  byte FW_major = SPI.transfer(0x00);
  byte FW_minor = SPI.transfer(0x00);
  digitalWrite(NSS_PIN, HIGH);

  Serial.println("---------------------------------------- ");
  Serial.print("Chip Version: ");
  Serial.print(FW_major, HEX); Serial.print(".");
  Serial.print(FW_minor, HEX); Serial.print(".");
  Serial.println(hw_ver, HEX);
}
...
void loop() {
  ...
  byte txTimeout[] = {0x00, 0x00, 0x00}; //disable timeout
  sendCommandVerified("SetTx", CMD_SET_TX, txTimeout, 3);

  Serial.println("\nTransmisión iniciada. Esperando 1 segundos...\n");
  delay(1000);
}