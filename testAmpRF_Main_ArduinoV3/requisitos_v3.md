...
* **Reloj del Microcontrolado:** 8MHz interno
...
Códigos de Identificación de Dispositivo (Disp.):
...
Manejo del Campo Argumento: 
* Para comandos de conmutación de RF (TX y RX), los bits individuales del byte definen de forma directa el estado lógico de los pines de selección.
...
*	Para Comando monitoreo (0x01):
	...
	*	Byte 2: Registro de Estado General de Alertas (OK = 0x01, Falla por Temperatura = 0xF0, Falla por Consumo = 0x08).
	*	Byte 3: 0x00
...
*	Temporización de Conmutación (CHANGEMODEDELAY de 200mS): El retardo de tiempo no bloqueante definido por la macro CHANGEMODEDELAY se debe aplicar estrictamente entre el estado Standby y el modo nuevo, asegurando la estabilización eléctrica antes de activar el nuevo camino de RF.
...
*	Standby -> Modo RX:
	...
	3.	Esperar tiempo de estabilización definido en macro ONLNADELAY (100mS).
...
*	Standby -> Apagado:
	...
	2.	Esperar tiempo de retraso definido en la macro POWEROFFDELAY (100mS).
